...
module PeerConnection
  ...
  BLOCK_SIZE  = 16384
  attr_accessor :torrent, :torrent_manager, :last_seen, :bitfield,
                :request_queue, :compact_id, :leeching_pieces
  ...
  def process_handshake(payload)
    ...
    peer_id   = payload[11]
    @peer_id  = peer_id
    @client.register_peer(self, info_hash, peer_id) unless @torrent
  end
end
