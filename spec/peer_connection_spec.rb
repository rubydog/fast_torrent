describe "PeerConnection" do
  ...
  before(:each) do
    ...
    @bitfield = mock(BitField, field: [255,255,255,255],
                                      to_packed_s: [255,255,255,255].pack('C*')).as_null_object
    @torrent = mock(Torrent, info_hash: INFO_HASH, peer_id: PEER_ID, piece_count: 120,
                    interested_in_peer?: false, read_block: "a" * 2**14, piece_size: 2**16,
                    bitfield: @bitfield, recalculate_chokes: true, peer_interest_changed: true,
                    sent_block: true, peer_choked_us: true)
    @client = fast_torrent.clone
    @peer_c = TestPeerConnection.new(@client)
    @peer_c.torrent_manager = @client
  ...
  context 'incoming connection' do
    context 'receiving handshake' do
      it "should try to register with the torrent manager" do
        ...
        @client.should_receive(:register_peer).with(@peer_c, INFO_HASH, PEER_ID)
